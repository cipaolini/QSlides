...
First of all, we computed the c-value (**Concordance index C: goodness of fit**) for the two models with fixed and random effects: even if the model with T+S performs better with the mixed effects, the higher c-v of 0.98 by the Tmodel suggests how the traditional p performs better alone, than in combination with the semantic ones.
:::
...
::: notes
Computating the delta c-value, we have been able to check the **Difference of fixed/random effects between models**: in what extent the contribution of the predictors change in the models?
(see with mariana)
:::
