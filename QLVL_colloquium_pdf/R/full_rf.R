...
traditional_predictors <- read_tsv(here::here("QLVL_colloquium", "data", "dat_us.tsv"), show_col_types = FALSE) %>% 
...
significant_dp <- read_tsv(here::here("QLVL_colloquium", "data_processed", "varimp_bin.tsv"),
         show_col_types = FALSE) %>% 
  filter(Varimp > 0) %>% 
  pull(Predictor)
clusters <- read_tsv(here::here("QLVL_colloquium", "data_processed", "dat_clusters.tsv"), #why taking the clusters?
         show_col_types = FALSE) %>% 
...
varimp_full %>% enframe(name = "Predictor", value = "Varimp") %>% 
  write_tsv(here::here("QLVL_colloquium", "data_processed", "varimp_full.tsv"))
