...
Let's consider only the group of recipients, here exemplified by the types government/nn and mother/nn. As you can see in this co-occurence matrix, each row represents a target-words (from the dtasate) from the recipient slot: the aggregation of the frequencies between the TW and the CW (from the corpus) constitutes a **word-type vector**. What you see here are raw frequencies, that is how many time the TW co-occur with the CW. However, -\>
:::
...
The values here represent the the attraction/the association strength between the target word and a context word.

It is important to acknowledge that building a DS model means training DS model workflow, a type-level one in this case, with different parameters and compare them to pick the best one BASED ON CUSTOMARY CRITERIA.
:::
