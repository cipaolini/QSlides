...
traditional_predictors <- read_tsv(here::here("Taal_tongval", "data", "dat_us.tsv"), show_col_types = FALSE) %>% 
...
significant_dp <- read_tsv(here::here("Taal_tongval", "data_processed", "varimp_bin.tsv"),
         show_col_types = FALSE) %>% 
  filter(Varimp > 0) %>% 
  pull(Predictor)
clusters <- read_tsv(here::here("Taal_tongval", "data_processed", "dat_clusters.tsv"), #why taking the clusters?
         show_col_types = FALSE) %>% 
